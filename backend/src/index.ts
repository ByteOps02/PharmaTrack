...
const patientSchema = z.object({
  ...
  dob: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
...
const vitalsSchema = z.object({
  patientId: z.number().min(1),
  recordDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
...
const medicationSchema = z.object({
  ...
  prescribedDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
...
const labResultSchema = z.object({
  patientId: z.number().min(1),
  testDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
...
const allergySchema = z.object({
  ...
  notes: z.string().optional(),
  recordedDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
  recordedBy: z.string().min(1),
})
...
const conditionSchema = z.object({
  ...
  diagnosedDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
  status: z.enum(["active", "inactive"]).optional(),
...
const invoiceSchema = z.object({
  ...
  invoiceDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
  dueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
...
const paymentSchema = z.object({
  ...
  paymentDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
...
const insuranceClaimSchema = z.object({
  ...
  submissionDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
  serviceDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Invalid date format, expected YYYY-MM-DD" }),
...
app.post("/api/auth/signup", loginLimiter, async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(users).values({ fullName, email, passwordHash }).returning()
...
app.put("/api/patients/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(patients).set(validation.data!).where(eq(patients.id, id)).returning()
...
app.put("/api/appointments/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(appointments).set(validation.data!).where(eq(appointments.id, id)).returning()
...
app.post("/api/clinical-records", async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(clinicalRecords).values(validation.data!).returning()
...
app.post("/api/vitals", async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(vitals).values(validation.data!).returning()
...
app.put("/api/vitals/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(vitals).set(validation.data!).where(eq(vitals.id, id)).returning()
...
app.post("/api/medications", async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(medications).values(validation.data!).returning()
...
app.put("/api/medications/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(medications).set(validation.data!).where(eq(medications.id, id)).returning()
...
app.post("/api/lab-results", async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(labResults).values(validation.data!).returning()
...
app.put("/api/lab-results/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(labResults).set(validation.data!).where(eq(labResults.id, id)).returning()
...
app.post("/api/allergies", async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(allergies).values(validation.data!).returning()
...
app.put("/api/allergies/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(allergies).set(validation.data!).where(eq(allergies.id, id)).returning()
...
app.post("/api/conditions", async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(conditions).values(validation.data!).returning()
...
app.put("/api/conditions/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(conditions).set(validation.data!).where(eq(conditions.id, id)).returning()
...
app.post("/api/invoices", async (req: Request, res: Response) => {
  try {
    ...
    const { patientId, appointmentId, invoiceDate, dueDate, subtotal, taxAmount, discountAmount, notes } =
      validation.data!
...
app.put("/api/invoices/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db.update(invoices).set(validation.data!).where(eq(invoices.id, id)).returning()
